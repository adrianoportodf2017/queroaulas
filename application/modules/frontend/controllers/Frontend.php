...
class Frontend extends MX_Controller
{
  ...
  public function checkout($payment_request = "only_for_mobile")
  {
    ...
    $data = array();
    $data['settings'] = $this->frontend_model->getSettings();
    $data['teacher'] = $this->teacher_model->getTeacherById($this->input->get('id'));
    $data['date'] =   $_GET['date'];
    $data['hour'] =   $_GET['hour'];
    $data['id'] =   $_GET['id'];
    $data['message'] =   '';
  ...
  function salvarClient()
  {
    ...
    if (isset($ion_user_id)) {
      ...
      $client_user_id = $this->db->get_where('client', array('email' => $_POST['email']))->row()->id;
      $id_info = array('ion_user_id' => $ion_user_id);
    ...
    //  var_dump($id);die;
      ...
      if ($this->ion_auth->login($this->input->post('email'), $this->input->post('password'))) {
        ...
        $clientname = $this->client_model->getClientById($client_user_id)->name;
        $client_phone = $this->client_model->getClientById($client_user_id)->phone;
