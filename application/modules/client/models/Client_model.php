...
class Client_model extends CI_model {
    ...
    function getClientWithTeacher($cliente, $teacher) {
        $this->db->where('ion_user_id',$cliente);
    ...
    function getClientWithoutSearch($order, $dir) {
        if ($order != null) {
    ...
    function getClientBySearch($search, $order, $dir) {
        if ($order != null) {
            $this->db->order_by($order, $dir);
        } else {
            $this->db->order_by('id', 'desc');
    ...
    function getClientByLimit($limit, $start, $order, $dir) {
        if ($order != null) {
            $this->db->order_by($order, $dir);
        } else {
            $this->db->order_by('id', 'desc');
    ...
    function getClientByLimitBySearch($limit, $start, $search, $order, $dir) {
    ...
    function getClientMaterialWithoutSearch($order, $dir) {
        if ($order != null) {
            $this->db->order_by($order, $dir);
        } else {
            $this->db->order_by('id', 'desc');
    ...
    function getClientInfo($searchTerm) {
        if (!empty($searchTerm)) {
            $this->db->select('*');
            $this->db->where("name like '%" . $searchTerm . "%' ");
            $this->db->or_where("id like '%" . $searchTerm . "%' ");
            $fetched_records = $this->db->get('client');
    ...
    function getClientinfoWithAddNewOption($searchTerm, $teacher) {
    
     //   var_dump( $teacher_id['0'] );die;
